...
with aba1:
    ...
    **Fique à vontade para explorar!**  
    ...
    df_filter = filter_dataframe(df_traduzido) 
    st.dataframe(
    ...
    fig1 = px.treemap(
        ...
        title= 'Treemap de emissão de CO<sub>2</sub>', #2 subescrito usando html
        labels = {
            'contagem': 'quantidade de veículos',
            'emissões co2 g/km': 'emissão de CO<sub>2</sub> (g/km)'
        },
        hover_data= {'emissões co2 g/km': ":.2f"},
        color_continuous_midpoint=np.average(df_filter['emissões co2 g/km'], weights=df_filter['contagem'])
    )
